...
public class OrderAction implements IAction {
    ...
    public void execute() {
        ...
        try {
            ...
            System.err.println("Critical error while testing OrderDAO: " + e.getMessage());
            e.printStackTrace();
        } finally {
            cleanupTestData();
        }
    ...
    private void testCreateOrder() {
        System.out.println("2. Creating order (id is specified via a setter).");

        try {
            if (orderDAO.existsById(orderId)) {
                System.out.println("Order ID=" + orderId + " already exists, using existing one");
                return;
            }
            ...
            var tech1 = new Technician("Technician 1", "Diagnostics", true);
            tech1.setId(technicianId1);

            var tech2 = new Technician("Technician 2", "Repair", true);
            tech2.setId(technicianId2);

            var testOrder = new Order(
                    advisor, Arrays.asList(tech1, tech2), customer, carPlace, OrderStatus.PENDING, 10000, LocalDateTime.now(),
            ...
            orderDAO.addTechnicianToOrder(orderId, technicianId2);
    ...
    private void testExistsById() {
        ...
        try {
            var exists = orderDAO.existsById(256);
        ...
        try {
            var exists = orderDAO.existsById(orderId);
    ...
    private void cleanupTestData() {
        ...
        try {
            orderDAO.removeTechnicianFromOrder(orderId, technicianId1);
            orderDAO.removeTechnicianFromOrder(orderId, technicianId2);
